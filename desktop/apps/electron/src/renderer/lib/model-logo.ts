...
import DefaultLogo from '@/assets/models/helios.png'
...
// Helios brand mark（helios.png；proma.png 为同图兼容路径）
import HeliosLogo from '@/assets/models/helios.png'
import PromaLogo from '@/assets/models/helios.png'
...
const MODEL_LOGO_MAP: Record<string, string> = {
  ...
  // === Helios（业务对话 model=helios；兼容旧 proma id）===
  helios: HeliosLogo,
  proma: HeliosLogo,
...
export function resolveModelDisplayName(modelId: string, channels: import('@proma/shared').Channel[]): string {
  if (/^helios$/i.test(modelId) || /^proma$/i.test(modelId)) {
    return 'Helios'
  }
...
/** 默认模型图标 */
export { DefaultLogo, PromaLogo, HeliosLogo }
