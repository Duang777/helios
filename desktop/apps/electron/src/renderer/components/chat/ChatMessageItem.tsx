...
import { channelsAtom, chatPendingMessageAtom } from '@/atoms/chat-atoms'
...
import { HeliosCards } from '@/components/helios/HeliosCards'
import { parseHeliosCards, stripHeliosFence } from '@/lib/helios/business-turn'
...
export const ChatMessageItem = React.memo(function ChatMessageItem({
  ...
  const heliosCards = React.useMemo(
    () => (message.role === 'assistant' ? parseHeliosCards(message.content) : null),
    [message.content, message.role],
  )
  const heliosText = React.useMemo(
    () => (heliosCards ? stripHeliosFence(message.content) : null),
    [heliosCards, message.content],
  )
            ...
            <>
              ...
              {message.content ? (
                <>
                  {heliosCards ? (
                    <>
                      {heliosText ? <MessageResponse>{heliosText}</MessageResponse> : null}
                      <HeliosCards
                        events={heliosCards}
                        busy={isStreaming}
                        onApprove={() => {
                          if (!conversationId) return
                          setChatPending({ conversationId, message: '批准' })
                        }}
                        onReject={() => {
                          if (!conversationId) return
                          setChatPending({ conversationId, message: '拒绝' })
                        }}
                        onConfirmIntent={() => {
                          if (!conversationId) return
                          setChatPending({ conversationId, message: '确认' })
                        }}
                        onCancelIntent={() => {
                          if (!conversationId) return
                          setChatPending({ conversationId, message: '取消' })
                        }}
                      />
                    </>
                  ) : (
