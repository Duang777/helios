...
function buildSkillClassificationPrompt(input: {
  ...
  return `请帮我整理当前项目在 Helios 工作区中保存的 Skills 的分组。
...
export function AgentSkillsView(): React.ReactElement {
  ...
  const handleClassifySkills = React.useCallback(async (): Promise<void> => {
    ...
    if (!data.skillsDir) {
      toast.error('无法定位当前项目的 Helios 工作区 Skills 目录')
      return
    }
...
function SkillsTab({
  ...
  if (total === 0) {
    return <EmptyState icon={<Blocks className="size-8 text-foreground/30" />} title="暂无 Skill" hint="可以在 Agent 模式下让 Helios 帮你联网查找并安装 Skill，或从其他项目导入。" />
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {builtinSkills.length > 0 && (
        <SkillSection title="Helios 内置" skills={builtinSkills} isBuiltin={isBuiltin} updatingSkill={updatingSkill} onOpen={onOpen} onToggle={onToggle} onUpdate={onUpdate} />
      )}
    </div>
  )
}
  ...
  if (total === 0) {
    return (
      <EmptyState
        ...
        title="还没有 MCP 服务器"
        hint="点击右上角「添加服务器」开始，或在 Agent 模式下让 Helios 帮你查找并配置。"
        action={
          <button
            type="button"
            onClick={onAdd}
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {builtinServers.length > 0 && (
        <McpSection title="Helios 内置" count={builtinServers.length}>
          {builtinServers.map((server) => (
            <McpCard
              ...
              entry={{
                type: 'stdio',
                command: 'Helios 运行时注入',
                enabled: server.enabled,
                isBuiltin: true,
              }}
              description={server.description}
              targetLabel={server.availabilityReason ?? 'Helios 运行时注入'}
