...
class FeishuBridge {
  ...
  /**
   * 为 Helios 桌面端会话准备飞书镜像群。
   *
   * 该群只包含用户与当前 Bot。用户在群里继续发送消息时，会通过
   * source=session-mirror 的绑定回到同一个 Helios session。
   */
  ...
  private async createNewSession(
    ...
    if (!workspaceId) {
      await this.sendMessage(chatId, '请先在 Helios 设置中创建项目。')
      return
    }
    ...
    if (!channelId) {
      await this.sendMessage(chatId, '请先在 Helios Agent 设置中选择渠道。')
      return
    }
  ...
  private async handleModelCommand(msgCtx: FeishuMessageContext, arg: string): Promise<void> {
    ...
    if (channels.length === 0) {
      await this.sendMessage(
        chatId,
        '暂无可用渠道。请先在 Helios 设置中配置并启用渠道（需填入 API Key 且至少启用一个模型）。',
      )
      return
    }
    ...
    if (!targetBinding) {
      ...
      if (!targetBinding) {
        await this.sendMessage(chatId, '请先发送一条消息创建会话，或在 Helios 设置中选择 Agent 渠道。')
        return
      }
    }
