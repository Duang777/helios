...
import {
  encodeHeliosMessage,
  handleBusinessTurn,
} from '@/lib/helios/business-turn'
import {
  getPendingApproval,
  setPendingApproval,
} from '@/lib/helios/pending-approval'
import {
  getPendingCompile,
  setPendingCompile,
} from '@/lib/helios/pending-compile'
import type {
  ChatMessage,
  ChatSendInput,
  FileAttachment,
...
function ChatViewInner({ conversationId }: ChatViewProps): React.ReactElement {
  ...
  const handleSend = React.useCallback(async (
    ...
    const heliosBusiness = store.get(heliosBusinessEnabledAtom)
    if (!heliosBusiness && !selectedModel) return
    ...
    if (isFirstMessage && content) {
      if (heliosBusiness) {
        const title = content.trim().slice(0, 40) || 'Helios 对话'
        void window.electronAPI.updateConversationTitle(conversationId, title).then((updated) => {
    ...
    setStreamingStates((prev) => {
      ...
      map.set(conversationId, {
        ...
        content: '',
        reasoning: '',
        model: heliosBusiness ? 'helios' : selectedModel!.modelId,
        toolActivities: [],
        startedAt: Date.now(),
      })
      return map
    ...
    // Helios 业务对话：旁路 Proma LLM，直连本机 API，并落盘会话
    if (heliosBusiness) {
      const userCreatedAt = Date.now()
      const persistUser = window.electronAPI
        ...
        .catch((err) => {
          console.error('[ChatView] Helios 用户消息落盘失败:', err)
        })

      try {
        const { events, pendingApproval, pendingCompile } = await handleBusinessTurn(
          finalContent,
          {
            pendingApproval: getPendingApproval(conversationId),
            pendingCompile: getPendingCompile(conversationId),
          },
        )
        setPendingApproval(conversationId, pendingApproval)
        setPendingCompile(conversationId, pendingCompile)
        const assistantContent = encodeHeliosMessage(events)
        const assistantMsg = {
          id: `helios-${Date.now()}`,
        ...
        await persistUser
        await window.electronAPI.appendMessage(conversationId, assistantMsg)
        // 侧边栏 updatedAt / 取消归档与磁盘对齐
        const list = await window.electronAPI.listConversations()
        setConversations(list)
      } catch (error) {
        console.error('[ChatView] Helios 业务对话失败:', error)
        await persistUser
        setChatStreamErrors((prev) => {
          const map = new Map(prev)
          map.set(
            conversationId,
            error instanceof Error ? error.message : 'Helios 请求失败',
          )
          return map
        })
      } finally {
  ...
  }, [chatPendingMessage, conversationId, selectedModel, isStreaming, handleSend, store, setChatPendingMessage])
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex flex-col h-full flex-1 min-w-0">
        ...
        <div className="flex flex-col flex-1 w-full max-w-[min(72rem,100%)] mx-auto overflow-hidden min-h-0">
          ...
          {/* Agent 模式推荐横幅（Helios 业务壳关闭） */}
          {!heliosBusiness && <AgentRecommendBanner />}
