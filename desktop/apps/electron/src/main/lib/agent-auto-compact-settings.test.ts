...
describe('Agent 自动压缩设置清理', () => {
  test('Given settings 含自动压缩窗口和开关 When 清理 Helios 自动压缩设置 Then 删除两个字段', () => {
  ...
  test('Given settings 只含自动压缩开关 When 清理 Helios 自动压缩设置 Then 删除开关字段', () => {
    const settings: Record<string, unknown> = {
  ...
  test('Given settings 没有自动压缩字段 When 清理 Helios 自动压缩设置 Then 不改动', () => {
    const settings: Record<string, unknown> = {
      plansDirectory: '.context',
      skipWebFetchPreflight: true,
    }
