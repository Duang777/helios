...
export function ChatInput({ conversationId, streaming, pendingAttachments, onSetPendingAttachments, onSend, onStop, onClearContext }: ChatInputProps): React.ReactElement {
  ...
  const canSend = (content.trim().length > 0 || pendingAttachments.length > 0)
    && (heliosBusiness || selectedModel !== null)
    && !streaming
  ...
  const handleSend = React.useCallback((): void => {
    if (!canSend) return
    // Helios 业务对话打本机 API，不依赖外网；Helios LLM 路径仍检查网络
    if (!heliosBusiness && !navigator.onLine) {
  ...
  }, [canSend, content, onSend, heliosBusiness])
  ...
  const toolbarItems = React.useMemo<ToolbarItem[]>(() => [
    ...(!heliosBusiness
      ? [{ key: 'model', node: <ModelSelector excludedProviders={['openai-codex']} useSharedOpenState /> } satisfies ToolbarItem]
      : []),
    ...(!heliosBusiness
      ? [{
      key: 'thinking',
  ...
  ], [handleOpenFileDialog, thinkingEnabled, setThinkingEnabled, onClearContext, heliosBusiness])
