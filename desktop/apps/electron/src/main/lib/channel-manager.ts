...
/** 保存 Pi 或 Helios 刷新后的完整 Codex OAuth 凭据。 */
...
export async function testChannel(channelId: string): Promise<ChannelTestResult> {
  ...
  try {
    switch (provider) {
      ...
      default:
        return { success: false, message: `不支持的供应商: ${provider}。你可能过去使用的是 Helios 商业版，请重新下载商业版覆盖安装，当前版本为开源版本。` }
    }
  } catch (error) {
 ...
 * Coding Plan 渠道必须发送 Helios User-Agent，否则返回 403。
 */
 ...
 * Coding Plan 渠道必须发送 Helios User-Agent。
