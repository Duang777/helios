...
/** Claude SDK 可读取的 settings 层级。Helios 只启用隔离的 user 层。 */
export type ClaudeSettingSource = 'user'

/**
 * 解析 Claude SDK 的 settings 来源。
 *
 * 未显式指定时同样只读取 CLAUDE_CONFIG_DIR 指向的 Helios 隔离配置，
...
/**
 * 所有项目均从 Helios 隔离配置读取 Claude SDK settings。
...
export interface ClaudeAgentQueryOptions extends AgentQueryInput {
  ...
  maxTurns?: number
  /** SDK 权限模式（Helios 当前三种模式直接映射 SDK 原生模式） */
  ...
  /** 仅使用 Helios 显式传入的 MCP 配置，避免 SDK 从其它来源发现额外 MCP */
  ...
  /** Helios 管理的会话级 settings.json；通过 SDK flag settings 显式加载。 */
  settingsFilePath?: string
  /** Claude SDK settings 来源。省略时仅读取 Helios 隔离的 user 配置。 */
...
export function mapSDKErrorToTypedError(
  ...
  const isInvalidChannelOrModel = /请检查是否选择了正确的 Helios 供应渠道和模型/.test(mapped.message)
  ...
  async *query(input: AgentQueryInput): AsyncIterable<SDKMessage> {
    ...
    try {
      ...
      const sdkOptions = {
        ...
        // 所有项目仅加载 Helios 隔离的 user 配置；会话 sidecar 由下方 settings 显式传入。
