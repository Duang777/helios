...
export function registerIpcHandlers(): void {
  ...
  // 追加单条消息（Helios 业务对话：不触发 LLM）
  ipcMain.handle(
    CHAT_IPC_CHANNELS.APPEND_MESSAGE,
    async (_, conversationId: string, message: ChatMessage): Promise<void> => {
      if (!conversationId || !message?.id || !message.role) {
        throw new Error('invalid appendMessage payload')
      }
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, runtime: AgentRuntime): Promise<AgentSessionMeta> => {
      ...
      if (previousRuntime !== runtime) {
        // 两套 runtime 的会话 artifact 不可互用；下一轮从 Helios 已持久化的转录恢复。
  ...
  ipcMain.handle('migration:openFileDialog', async () => {
    ...
    const result = await dialog.showOpenDialog({
      title: '选择迁移文件',
      filters: [
        { name: 'Helios 迁移文件', extensions: ['proma-backup', 'proma-share'] },
        { name: '所有文件', extensions: ['*'] },
      ],
      properties: ['openFile'],
    })
  ...
  ipcMain.handle('migration:saveFileDialog', async (_, mode: string) => {
    ...
    const result = await dialog.showSaveDialog({
      ...
      defaultPath: defaultName,
      filters: [
        { name: mode === 'personal' ? 'Helios 个人备份' : 'Helios 分享包', extensions: [ext] },
      ],
    })
    return result.canceled ? null : result.filePath
  })
