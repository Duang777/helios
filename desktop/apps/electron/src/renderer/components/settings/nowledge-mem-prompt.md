请帮我从零完成 Nowledge Mem 接入 Helios 的完整配置。严格按以下步骤操作。

---

## Step 0：确认 Nowledge Mem 客户端已安装并运行（必须）

本地模式下**不需要登录或注册账号**——Nowledge Mem 是本地优先的，App 装好并启动之后，后台服务就在 `127.0.0.1:14242` 运行，Helios 通过 MCP 直连，全程不出本地。
...
## Step 0.5：与用户确认目标 Helios 工作区（必须）

MCP、Skills 按 Helios 工作区生效。Hooks 写在 Helios 的全局 Claude 配置中，但 Helios 会在每次会话启动时注入当前项目的环境变量；Hooks 只会在 `nowledge-mem` 已启用的项目运行。因此**必须先问清楚用户希望配置到哪个 Helios 工作区**。
...
- `question`: "你希望把 Nowledge Mem 配置到哪个 Helios 工作区？"
- `header`: "目标 Helios 工作区"
- `multiSelect`: `false`
...
## Step 2：下载插件文件并安装到目标 Helios 工作区
```bash
...
Hooks 是全局 Claude 配置，**不要在命令中硬编码某个项目或 Helios 工作区目录**。Helios 会在每次会话启动时提供 `PROMA_HOME`、`PROMA_WORKSPACE_DIR` 和 `PROMA_NOWLEDGE_MEM_ENABLED`；下面的条件会使 Hook 仅在已启用 `nowledge-mem` 的项目中运行。
...
## ⚠️ 以上步骤完成后，必须完全退出并重启 Helios，配置才会生效。

MCP 和 Hooks 只在 Helios 启动时加载，不重启等于没有配置。
