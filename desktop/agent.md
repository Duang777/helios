# desktop/ — Helios 桌面壳（Proma 魔改）

- 来源与许可：`UPSTREAM.md`、`.proma-source`、根目录 ADR-004；`LICENSE`（AGPL-3.0）
...
- 业务对话：`heliosBusinessEnabledAtom`（默认开）→ `ChatView` 旁路 Proma LLM → `renderer/lib/helios/`，消息经 `appendMessage` 落盘
...
- 不把 Proma Agent SDK 当 Helios runtime
