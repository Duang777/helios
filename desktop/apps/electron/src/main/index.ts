...
if (!app.requestSingleInstanceLock()) {
  console.warn(
    '[启动] 已有 Helios/Helios 进程持有单实例锁，本次启动将退出。\n' +
      '  如果窗口未出现，可能旧进程已卡死。请运行 `killall Electron` 或结束占用进程后重试。',
  )
  app.quit()
} else {
  // 主流程：正常启动（单实例锁已获取）
...
async function bootstrap(): Promise<void> {
  // 初始化 Helios 版本号（供 User-Agent 等全局标识使用）
  ...
  try {
    ...
    dialog.showErrorBox(
      ...
      `部分功能可能不可用：\n\n${message}\n\n` +
        ...
        `常见原因与排查：\n` +
        `1. 旧版 Helios 进程未退出（终端运行 killall Helios 后重试）\n` +
