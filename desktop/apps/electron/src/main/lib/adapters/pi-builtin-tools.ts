...
function buildWebTools(sdk: PiSdk): ToolDefinition[] {
  return [
    sdk.defineTool({
      name: 'WebSearch',
      label: '搜索网页',
      description: 'Search the web for up-to-date information through Helios\'s Tavily integration. Use for current events, recent data, facts that may be stale, or when the user explicitly asks to search.',
    ...
    sdk.defineTool({
      name: 'WebFetch',
      label: '抓取网页',
      description: 'Fetch and extract readable Markdown content from a URL through Helios\'s Tavily integration. Use after WebSearch or when the user gives a URL and asks to inspect page content.',
...
function buildAutomationTools(sdk: PiSdk, ctx: PiBuiltinToolsContext): ToolDefinition[] {
  return [
    sdk.defineTool({
      ...
      label: '列出定时任务',
      description: '列出 Helios 持久化定时任务。用于查看已有长期反复任务、判断是否需要新建任务、检查运行状态和最近失败情况。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '查看定时任务',
      description: '读取单个 Helios 定时任务详情和运行记录。定时任务自动执行中可以省略 id 来读取当前任务，用于自检和自迭代。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '创建定时任务',
      description: '创建 Helios 持久化定时任务。适合无人值守、有稳定价值的场景。纯提醒/闹钟、需要用户实时参与判断、或现在就该做完即终结的事不要创建。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '修改定时任务',
      description: '修改 Helios 定时任务，包括名称、执行提示词、频率和启用状态。定时任务自动执行中可以省略 id 来修改当前任务。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '删除定时任务',
      description: '删除 Helios 定时任务。只在用户明确要求删除，或任务已经长期无价值且用户确认后使用。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      label: '立即运行定时任务',
      description: '立即运行 Helios 定时任务。用于用户要求马上验证，或修改任务后需要试跑一次。',
      parameters: Type.Object({
...
function buildPlanningTools(sdk: PiSdk, ctx: PiBuiltinToolsContext): ToolDefinition[] {
  ...
  return [
    sdk.defineTool({
      ...
      description: '列出 Helios 本地 Todo。适合在安排工作、检查今天待办、维护任务状态前使用。仅 Pi Agent 可用。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      description: '创建 Helios 本地 Todo。调用前必须先用 list_todos(status=open) 检查重复，并用 list_groups({ scope: todo }) 查询并优先复用 Todo 分组；用户明确提出待办，或可合理确定下一步时使用。未传 dueAt 时默认当天结束前；仅 Pi Agent 可用。',
    ...
    sdk.defineTool({
      ...
      description: '列出 Helios 本地日程。用于查看指定时间范围的安排。仅 Pi Agent 可用。',
      parameters: Type.Object({
    ...
    sdk.defineTool({
      ...
      description: '创建 Helios 本地日程。分组必须来自 list_groups({ scope: calendar })；用户明确提供时间安排时使用。仅 Pi Agent 可用。',
    ...
    sdk.defineTool({
      ...
      description: '删除 Helios 本地日程。只在用户明确要求删除时使用。仅 Pi Agent 可用。',
      parameters: Type.Object({ id: Type.String() }),
      async execute(_id: string, params: unknown) {
        assertPlanningDeleteAllowed(ctx)
        const id = assertNonBlank((params as { id: string }).id, 'id')
...
function buildPromaCloudTools(sdk: PiSdk, _ctx: PiBuiltinToolsContext): ToolDefinition[] {
  // proma-cloud MCP 工具（get_credentials / create_app_key）通常由 Helios 的
