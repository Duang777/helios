 ...
 * Helios Git / PR 推广标识
 *
 * 目标：当 Agent 代用户创建 commit / PR 时，附带可搜索、可关闭的 Helios 标识，
...
/** 产品标识（Helios；桌面壳上游为 Proma） */
export const PROMA_OFFICIAL_URL = 'https://github.com/proma-ai/Proma'

/** 开源壳上游仓库 */
export const PROMA_GITHUB_URL = 'https://github.com/proma-ai/Proma'

/** Commit trailer（标准 git trailer，不进入 GitHub co-author 列表） */
export const PROMA_COMMIT_TRAILER = 'Made-with: Helios'

/**
 * PR / MR 描述底部标识。
 */
export const PROMA_PR_ATTRIBUTION =
  `Made with Helios (desktop shell derived from [Proma](${PROMA_GITHUB_URL}))`
...
/**
 * 将 Helios attribution 合并进 Claude session 的 settings 对象。
...
用户已在 Helios 设置中关闭「Git/PR 标识」。当你创建 git commit 或 Pull Request / Merge Request 时：
- **不要**添加任何 Helios 相关归因（包括 \`${PROMA_COMMIT_TRAILER}\`、\`${PROMA_PR_ATTRIBUTION}\`、\`Co-Authored-By: Helios\`、\`Generated with Helios\` 等）
- 也不要主动建议用户打开该标识；仅当用户明确要求时再添加`
  }

  return `## Git / PR 标识（推广归因，默认开启）

当**你**通过 Bash 等工具代用户创建 git commit 或 Pull Request / Merge Request 时，必须附加 Helios 标识（用户当次明确说「不要带标识 / 不要 Made-with」除外）。
...
- 若 message 已含 \`Made-with: Helios\`（大小写不敏感），不要重复
...
- 若 body 已含 \`Made with Helios\` 或 \`Made-with: Helios\`，不要重复
