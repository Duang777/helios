...
function buildWorkspaceMemoryInitPrompt(historyRange: MemoryHistoryRange): string {
  ...
  return `请为当前项目初始化并沉淀长期记忆。这里的“项目”指系统提示中的“项目根目录”及其关联的 Agent 工作会话；不要把 Helios 工作区笼统当作项目。
...
路径与职责边界：
- 系统提示中的“Helios 工作区目录”是 Helios 管理配置与隔离资料的位置，存放 MCP、Skills、Helios 管理的 CLAUDE.md 与 Auto Memory；它不是用户项目根目录。必须按系统提示给出的绝对路径操作，不得猜测或替换路径。
...
2. 只更新系统提示明确给出的“Helios 工作区 CLAUDE.md”绝对路径。这里是 Helios 管理的项目指令文件；内容仅限稳定、跨会话有效的项目规则、入口和工作方法，不得混入临时调试、聊天记录或长篇资料。
3. 只更新系统提示明确给出的“Helios 工作区 Auto Memory 目录”中的 \`MEMORY.md\`、必要的主题文件和 \`user-profile.md\`，不要在其他目录创建记忆文件。\`MEMORY.md\` 保持简短的主题索引与路由，主题细节拆分到主题文件。
        ...
        <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
              新建一个 Agent 会话，读取当前项目{historyRangeLabel}的工作会话，沉淀并更新 Helios 工作区中的 CLAUDE.md 与 auto memory 文件。
            </div>
          </div>
