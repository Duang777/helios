...
| Q2 | Workflow 主语言长期是否坚持 YAML，还是 YAML+TS 双主？ | 影响编译器与用户技能模型 | 开放 |
| Q3 | Pi 集成用 RPC 子进程还是长期 TS 微服务？ | 部署形态 | 开放（当前 sidecar） |
| Q4 | 证据默认本地文件还是 DB？ | 运维与检索 | 开放（当前本地文件） |
| Q5 | 目标部署：仅本地单机，还是内网服务？ | 安全模型 | 开放 |
| Q6 | 开源策略与品牌边界（相对 helios.ai 名称） | 法务与传播 | 开放 |
