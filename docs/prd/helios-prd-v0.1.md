...
6. 工作流以 **可版本化制品**为一等公民：**YAML 定义 DAG/契约**；**TS 做节点实现与工具链**；不另开 TS-first workflow 语言（ADR-003 / Q2）。  
7. 第一条真实业务平台与剧本：**飞书 + `feishu.daily-brief`**（Q1 已关闭，Slice M；真人验收见 `docs/acceptance/2026-08-01-feishu-live.md`）。Demo CLI 仍用于结构回归。  
8. Pi 以 **本机 HTTP sidecar** 集成（Q3）；证据默认 **本地文件**（Q4）；部署目标 **本地单机**（Q5）。详见 ADR-003。
...
**真实平台（PRD Q1 已关闭）：飞书 / Lark**

| 剧本 | 说明 |
|------|------|
| **`feishu.daily-brief`** | 试点：今日日程 → 审批 → 发 IM 简报（`docs/architecture/slice-m-feishu-daily-brief.md`） |
| 其它 | `docs/feishu-cli.md` 中的 doctor / agenda / send-text 等 |

**结构占位（模拟，仍保留）：** `demo.lead-sync`
...
| # | 问题 | 影响 | 状态 |
|---|---|---|---|
| Q1 | 第一条真实业务平台与剧本是什么？ | 决定 CLI 设计与 demo 说服力 | **已关闭：飞书 + `feishu.daily-brief`**（Slice M） |
| Q2 | Workflow 主语言长期是否坚持 YAML，还是 YAML+TS 双主？ | 影响编译器与用户技能模型 | **已关闭：YAML 定义 DAG/契约；TS 做实现与工具链；非双主语言**（ADR-003） |
| Q3 | Pi 集成用 RPC 子进程还是长期 TS 微服务？ | 部署形态 | **已关闭：本机 HTTP sidecar**（ADR-001 / ADR-003） |
| Q4 | 证据默认本地文件还是 DB？ | 运维与检索 | **已关闭：本地文件系统**（ADR-003） |
| Q5 | 目标部署：仅本地单机，还是内网服务？ | 安全模型 | **已关闭：本地单机**（ADR-003） |
| Q6 | 开源策略与品牌边界（相对 helios.ai 名称） | 法务与传播 | **开放**（挡完整产品定稿；不挡 MVP 技术定稿） |
...
| 0.1.2 | 2026-08-01 | Q2–Q5 按现状冻结（ADR-003）；状态改为 MVP 技术定稿；仅 Q6 仍开放 |
| 0.1.2a | 2026-08-01 | 澄清 Q2：YAML 编排 + TS 实现/工具链，不是禁 TS，也不是双主语言 |
