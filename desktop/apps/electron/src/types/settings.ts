...
/** 将当前识别结果作为 Helios 输入框中的临时组合文本预览。 */
...
export interface AppSettings {
  ...
  /**
   * Helios 品牌主题一次性迁移标记。
   * 未设置时将 themeMode 收敛为 light（避开 Proma 遗留的 dark 默认）。
   */
  heliosBrandThemeV1?: boolean
  /** 界面风格 */
  ...
  /** 飞书 Session 镜像设置：每个 Helios Session 可创建一个仅包含用户与指定 Bot 的飞书群 */
  feishuSessionMirror?: FeishuSessionMirrorSettings
  /** 用户手动关闭的 Helios 内置 MCP ID 列表（针对默认开启的内置 MCP） */
  builtinMcpDisabledIds?: string[]
  /** 用户手动开启的 Helios 内置 MCP ID 列表（针对默认关闭的内置 MCP，如 nano-banana、mem） */
  ...
  /**
   * Agent 代创建 git commit / PR 时是否附加 Helios 推广标识。
   * 默认 true：commit trailer `Made-with: Helios`。
   * 关闭后不注入任何 Helios 归因，并覆盖 Claude SDK 默认 Co-Authored-By。
   */
