 ...
 * 打包环境下：asar 内的路径需要转换为 asar.unpacked 路径（即便 Helios 当前 `asar: false`
 * 兜底不伤人）。
 */
...
这些目录已由 Helios 授权给当前会话，和当前工作目录同属于用户允许访问的范围。
...
function createLocalProjectRootUnavailableError(projectRootPath: string, status?: string): Error {
  const error = new Error(
    `本地项目根目录不可用: 本地项目根目录不存在或无法访问：${projectRootPath}。请在 Helios 中重新选择项目文件夹。`,
...
export class AgentOrchestrator {
  ...
  private buildSdkRuntimeEnv(
    ...
    const sdkEnv: Record<string, string | undefined> = {
      ...
      // Helios 自行管理工具呈现和 MCP 连接，不依赖此机制。
      ...
      // Helios 的全局配置目录与当前项目状态，供 Claude Hooks 等受控子进程使用。
      PROMA_HOME: getConfigDir(),
      PROMA_NOWLEDGE_MEM_ENABLED: '0',
    }

    // 认证方式按 provider 分支
    // - Coding Plan / Token Plan：只认 Bearer，通过 ANTHROPIC_CUSTOM_HEADERS 注入 Helios UA
  ...
  async sendMessage(input: AgentSendInput, callbacks: SessionCallbacks): Promise<void> {
    ...
    if (workspaceId) {
      ...
      if (projectRootStatus && projectRootStatus !== 'available') {
        reportPreflightError({
          ...
          title: '本地项目根目录不可用',
          message: `本地项目根目录不存在或无法访问：${workspace.projectRootPath}。请在 Helios 中重新选择项目文件夹。`,
    ...
    try {
          ...
          actions: [
            {
              ...
              payload: 'https://github.com/proma-ai/Proma/download',
            },
            {
              key: 'i',
              label: '报告问题',
      ...
      // Pi 路径由 Helios 主进程连接用户 MCP server，并转换为 Pi customTools。
      ...
      // Claude 会话统一使用 Helios sidecar settings，不加载项目根 .claude。
      ...
      const queryOptions: ClaudeAgentQueryOptions | PiAgentQueryOptions = agentRuntime === 'pi' ? {
        ...
        // buildSystemPrompt 追加 Helios 特有指令（角色定义、子 Agent 委派策略、工作区信息等）
        systemPrompt: {
          type: 'preset',
          preset: 'claude_code',
        ...
        // Helios 统一使用 collaboration 派生子会话承载子 Agent 委派，避免 SDK 临时
        // Agent/Task 与 Helios 会话体系分裂。
      ...
      for (let attempt = 1; attempt <= MAX_AUTO_RETRIES + 1; attempt++) {
        ...
        try {
          ...
          while (true) {
            ...
            if (msg.type === 'assistant' && !isPartialMessage) {
              ...
              if (assistantMsg.error) {
                ...
                // 先自动清除 SDK resume 关系，改用 Helios 已持久化上下文重跑一次；再失败才展示用户提示。
                if (
                ...
                // 自动清除 resume 指针，改用 Helios 最近历史回填重跑一次；用于飞书/自动任务等无人值守入口自恢复。
                if (
          ...
          // 上下文过长：清除超限 resume 指针，用 Helios 历史回填自动恢复一次。
   ...
   * 运行中动态切换会话的权限模式
   *
   * 同时更新 Helios 侧（canUseTool 闭包读取的 Map）和 SDK 侧（query.setPermissionMode）。
   ...
   * 2. 截断 Helios JSONL 到 assistantMessageUuid（inclusive）
