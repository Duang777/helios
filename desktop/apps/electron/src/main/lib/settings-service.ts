...
export function getSettings(): AppSettings {
  ...
  try {
    ...
    // Helios：一次性把 Proma 遗留的 dark 默认切到 Claude 纸感浅色
    const needsBrandTheme = data.heliosBrandThemeV1 !== true
    const themeMode = needsBrandTheme
      ? DEFAULT_THEME_MODE
      : (data.themeMode || DEFAULT_THEME_MODE)
    const themeStyle = needsBrandTheme ? 'default' : data.themeStyle

    const resolved: AppSettings = {
      ...settings,
      themeMode,
      themeStyle,
      heliosBrandThemeV1: true,
    ...
    if (needsBrandTheme) {
      try {
        writeFileSync(filePath, JSON.stringify(resolved, null, 2), 'utf-8')
        console.log('[设置] Helios 品牌主题已迁移为 light')
      } catch (error) {
        console.error('[设置] Helios 品牌主题迁移写入失败:', error)
      }
    }
